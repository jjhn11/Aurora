...
const totalSlides = computed(() => Math.ceil(culturalEvents.value.length / 3));
...
const eventGroups = computed(() => {
  const groups = [];
  for (let i = 0; i < culturalEvents.value.length; i += 3) {
    groups.push(culturalEvents.value.slice(i, i + 3));
  }
  return groups;
});
...
<template>
    <div class="contenedor-carrusel container-fluid d-flex justify-content-center px-0">
        <!-- Botones laterales (pantallas grandes) -->
        <button
        ...
        <div id="carrusel1" class="carousel slide" data-bs-touch="true">
    ...
    <!-- Botones móviles (también modificados) -->
    <div class="text-center mobile-controls" :class="{ 'invisible': totalSlides <= 1 }">
      ...
      <button class="btn btn-outline-primary mx-2" type="button" data-bs-target="#carrusel1" data-bs-slide="next">
